...
export function EntityModal({ entity, customers, cars, onClose, onSaved }: Props) {
  ...
  return <Modal title={title} onClose={onClose}>
    <form ref={formRef} className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
      ...
      {entity.kind === "order" && <>
        <Field label="Автомобиль" full><div className="grid gap-2"><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted" size={18}/><input className={`${inputClass} pl-10`} value={carQuery} onChange={(event) => setCarQuery(event.target.value)} placeholder="Клиент, телефон, марка, номер или VIN" /></div><input name="car_id" type="hidden" value={selectedCarId} required />{selectedCarId && !carQuery && (() => { const selected = cars.find((car) => String(car.id) === selectedCarId); const owner = customers.find((customer) => customer.id === selected?.customer_id); return selected ? <button type="button" className="rounded-xl border border-apex/40 bg-apex/10 p-3 text-left text-sm text-apex" onClick={() => setSelectedCarId("")}><strong className="block">{carName(selected)}</strong><span className="text-xs text-muted">{owner?.full_name || "Клиент не указан"}{owner?.phone ? ` · ${owner.phone}` : ""} · изменить</span></button> : null; })()}<div className={`${carQuery.trim() ? "grid" : "hidden"} max-h-52 gap-1 overflow-y-auto rounded-xl border border-line bg-canvas p-1`}>{cars.filter((car) => { const owner = customers.find((customer) => customer.id === car.customer_id); const haystack = `${carName(car)} ${owner?.full_name ?? ""} ${owner?.phone ?? ""}`.toLocaleLowerCase("ru"); return haystack.includes(carQuery.trim().toLocaleLowerCase("ru")); }).slice(0, 20).map((car) => { const owner = customers.find((customer) => customer.id === car.customer_id); return <button key={car.id} type="button" className="rounded-lg p-3 text-left hover:bg-panel-soft" onClick={() => { setSelectedCarId(String(car.id)); setCarQuery(""); }}><strong className="block">{carName(car)}</strong><span className="text-xs text-muted">{owner?.full_name || "Клиент не указан"}{owner?.phone ? ` · ${owner.phone}` : ""}</span></button>; })}</div>{!selectedCarId && !carQuery.trim() && <p className="text-xs text-muted">Начните вводить данные автомобиля или клиента.</p>}{carQuery.trim() && !cars.some((car) => { const owner = customers.find((customer) => customer.id === car.customer_id); return `${carName(car)} ${owner?.full_name ?? ""} ${owner?.phone ?? ""}`.toLocaleLowerCase("ru").includes(carQuery.trim().toLocaleLowerCase("ru")); }) && <p className="rounded-xl border border-line bg-canvas p-3 text-sm text-muted">Автомобиль не найден.</p>}</div></Field>
