...
export function LaborStandardsPage() {
  ...
  const [priceMode, setPriceMode] = useState<"market" | "rate">("rate"); const [orderId, setOrderId] = useState("");
  ...
  useEffect(() => {
    const remembered = localStorage.getItem(LAST_ORDER_KEY);
    if (remembered && orders.some((order) => String(order.id) === remembered)) setOrderId(remembered);
    else if (orders.length === 1) setOrderId(String(orders[0]!.id));
  }, [orders]);
