...
function Analytics({ totals, previous, change, margin, chart, attention, scheduled, openDetail }: { totals: ReturnType<typeof calculateFinance>; previous: ReturnType<typeof calculateFinance>; change: number | null; margin: number; chart: DailyFinance[]; attention: Order[]; scheduled: { id: number }[]; openDetail: AppOutlet["openDetail"] }) {
  ...
  return <div className="grid gap-4">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.05fr_1.35fr]">
      <RevenueDonut totals={totals} partsPercent={partsPercent} />
...
function RevenueDonut({ totals, partsPercent }: { totals: ReturnType<typeof calculateFinance>; partsPercent: number }) { const laborPercent = 100 - partsPercent; return <Card className="border-line/80 bg-gradient-to-br from-panel to-panel-soft"><div className="flex items-center justify-between"><h2 className="text-base font-black">Структура выручки</h2><span className="text-xs text-muted">Все источники</span></div><div className="mt-4 flex items-center gap-5"><div className="grid size-32 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(#4f7cff 0 ${laborPercent}%, #35d39a ${laborPercent}% 100%)` }}><div className="grid size-[92px] place-items-center rounded-full bg-panel text-center"><span><strong className="block text-base">{money(totals.revenue)}</strong><small className="text-muted">всего</small></span></div></div><dl className="min-w-0 flex-1 space-y-3 text-sm"><LegendRow color="bg-info" label="Работы" percent={laborPercent} value={totals.labor} /><LegendRow color="bg-success" label="Запчасти" percent={partsPercent} value={totals.partsRevenue} /><div className="border-t border-line pt-3"><LegendRow color="bg-apex" label="Наценка" value={totals.markup} /></div></dl></div></Card>; }

function LegendRow({ color, label, percent, value }: { color: string; label: string; percent?: number; value: number }) { return <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2"><span className={`size-2 rounded-full ${color}`} /><dt className="truncate text-muted">{label}{percent !== undefined ? ` · ${percent}%` : ""}</dt><dd className="font-bold">{money(value)}</dd></div>; }
