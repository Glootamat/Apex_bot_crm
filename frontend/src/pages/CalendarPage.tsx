...
export function CalendarPage() {
  ...
  return <div className="grid gap-5">
    ...
    <Card className="overflow-hidden p-0"><div className="flex items-center justify-between border-b border-line p-2 sm:p-3"><Button variant="ghost" className="size-10 p-0" onClick={() => setMonth((value) => new Date(value.getFullYear(), value.getMonth() - 1, 1))}>←</Button><strong className="capitalize">{month.toLocaleDateString("ru-RU", { month: "long", year: "numeric" })}</strong><Button variant="ghost" className="size-10 p-0" onClick={() => setMonth((value) => new Date(value.getFullYear(), value.getMonth() + 1, 1))}>→</Button></div><div className="grid grid-cols-7 gap-px bg-line text-center text-[10px] text-muted">{"Пн Вт Ср Чт Пт Сб Вс".split(" ").map((day) => <span key={day} className="bg-panel py-1.5 font-bold sm:py-2">{day}</span>)}{calendarDays.map((day, index) => <button key={index} type="button" aria-label={`${day.date.toLocaleDateString("ru-RU")}${day.items.length ? `, записей: ${day.items.length}` : ", создать запись"}`} onClick={() => day.items[0] ? openDetail({ kind: "appointment", value: day.items[0] }) : openEntity({ kind: "appointment", startsAt: `${day.date.toLocaleDateString("sv-SE")}T10:00` })} className={`relative min-h-11 bg-canvas p-1 text-center transition hover:bg-panel-soft sm:min-h-16 sm:text-left ${day.current ? "" : "text-muted/40"}`}><span className="block text-xs sm:text-right">{day.date.getDate()}</span><span className={`mx-auto mt-1 block size-1.5 rounded-full sm:hidden ${day.items.length ? "bg-info" : "bg-transparent"}`} />{day.items.slice(0, 2).map((item) => <span key={item.id} className="mt-0.5 hidden truncate rounded bg-info/15 px-1 text-[9px] font-bold text-info sm:block">{item.starts_at.slice(11, 16)} {item.brand}</span>)}{day.items.length > 0 && <span className="absolute bottom-0.5 right-1 text-[8px] font-bold text-info sm:hidden">{day.items.length}</span>}{day.items.length > 2 && <span className="hidden px-1 text-[9px] text-apex sm:block">ещё {day.items.length - 2}</span>}</button>)}</div><p className="border-t border-line px-3 py-2 text-xs text-muted">Нажмите пустой день, чтобы сразу создать запись на выбранную дату.</p></Card>
