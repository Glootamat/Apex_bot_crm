...
export function CarHistoryPage() {
  ...
  return <div className="grid min-w-0 gap-5">
    ...
    <section className="relative grid gap-3 before:absolute before:bottom-4 before:left-6 before:top-4 before:w-px before:bg-line">{events.length ? events.map((event) => <HistoryItem key={`${event.kind}-${event.value.id}`} event={event} onOpenOrder={(order) => openDetail({ kind: "order", value: order })} onOpenDiagnostic={(id) => navigate(`/diagnostics/${id}`)} />) : <div className="relative z-10"><EmptyState>В этой категории пока нет записей</EmptyState></div>}</section>
  </div>;
}
