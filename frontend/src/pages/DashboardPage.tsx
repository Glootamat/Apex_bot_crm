import { CalendarPlus, CarFront, CircleDollarSign, ClipboardPlus, UserPlus } from "lucide-react";
...
export function DashboardPage() {
  ...
  const workspaceName = account.data?.organization_name?.trim() || "Рабочее пространство";
  return <div className="grid gap-7">
    <header><p className="text-sm font-bold tracking-wide text-apex">{workspaceName}</p><h1 className="mt-1 text-3xl font-black sm:text-4xl">{greeting}</h1><p className="mt-2 text-muted">Рабочее пространство под контролем</p></header>
    ...
    <section><h2 className="mb-3 text-lg font-black">Быстрые действия</h2><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3"><Button onClick={() => openEntity({ kind: "order" })}><ClipboardPlus size={18} />Новый заказ</Button><Button variant="secondary" onClick={() => openEntity({ kind: "appointment" })}><CalendarPlus size={18} />Новая запись</Button><Button variant="secondary" onClick={() => openEntity({ kind: "customer" })}><UserPlus size={18} />Новый клиент</Button></div></section>
