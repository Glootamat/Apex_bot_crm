...
export function DashboardPage() {
  ...
  return <div className="grid gap-5 sm:gap-6">
    <header className="pt-1"><p className="text-lg font-black tracking-tight"><span className="text-apex">{workspaceName.split(" ")[0]}</span>{workspaceName.includes(" ") && <span> {workspaceName.split(" ").slice(1).join(" ")}</span>}</p><h1 className="mt-4 text-2xl font-black sm:text-3xl">{greeting}{name ? `, ${name}!` : "!"}</h1><p className="mt-1 text-sm text-muted">Рабочее пространство под контролем</p></header>
