...
export function DashboardPage() {
  ...
  return <div className="grid gap-5 sm:gap-6">
    ...
    <section><h2 className="mb-3 text-sm font-black sm:text-center">Быстрые действия</h2><div className="mx-auto grid w-full max-w-5xl grid-cols-4 gap-2 sm:gap-3"><Action icon={<Plus size={21} />} label="Новый заказ" tone="apex" onClick={() => openEntity({ kind: "order" })} /><Action icon={<CalendarPlus size={20} />} label="Новая запись" tone="info" onClick={() => openEntity({ kind: "appointment" })} /><Action icon={<UserPlus size={20} />} label="Новый клиент" tone="purple" onClick={() => openEntity({ kind: "customer" })} /><Action icon={<ClipboardCheck size={20} />} label="Диагностика" tone="success" onClick={() => void navigate("/diagnostics")} /></div></section>
...
function Action({ icon, label, tone, onClick }: { icon: React.ReactNode; label: string; tone: "apex" | "info" | "purple" | "success"; onClick: () => void }) { const tones = { apex: "bg-apex text-black", info: "bg-info/20 text-info", purple: "bg-purple-400/20 text-purple-300", success: "bg-success/20 text-success" }; return <button type="button" className="group grid min-w-0 justify-items-center gap-1.5 rounded-xl p-1.5 text-center transition hover:bg-panel-soft sm:min-h-28 sm:content-center sm:gap-2 sm:border sm:border-line sm:bg-panel sm:p-4 sm:hover:-translate-y-0.5 sm:hover:border-apex/50" onClick={onClick}><span className={`grid size-11 place-items-center rounded-2xl transition group-hover:scale-105 sm:size-12 ${tones[tone]}`}>{icon}</span><span className="text-[10px] font-semibold leading-tight text-muted sm:text-sm sm:text-white">{label}</span></button>; }
