import { CalendarPlus, CarFront, ClipboardCheck, ClipboardPlus, Pencil, Plus } from "lucide-react";
...
export function CarsPage() {
  ...
  return <div className="grid min-w-0 gap-5">
    ...
    <div className="grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-3">{items.length ? items.map((item) => {
      ...
      return <Card key={item.id} className="cursor-pointer transition hover:border-apex/50" onClick={() => openDetail({ kind: "car", value: item })}>
        <div className="flex min-w-0 items-start gap-3"><div className="grid size-12 shrink-0 place-items-center rounded-xl bg-apex/10 text-apex"><CarFront /></div><div className="min-w-0 flex-1"><h2 className="break-words text-lg font-black">{item.brand} {item.model}</h2><p className="break-words text-sm text-muted">{item.year || "Год не указан"}{item.plate_number ? ` · ${item.plate_number}` : ""}</p></div><Button variant="ghost" className="size-11 shrink-0 p-0" onClick={(event) => { event.stopPropagation(); openEntity({ kind: "car", value: item }); }} aria-label={`Изменить ${item.brand} ${item.model}`}><Pencil size={18} /></Button></div>
        <dl className="my-4 grid min-w-0 grid-cols-2 gap-2 rounded-xl bg-panel-soft p-3 text-sm"><div className="min-w-0"><dt className="text-muted">Владелец</dt><dd className="mt-1 break-words font-semibold">{customerName(owner?.full_name)}</dd></div><div><dt className="text-muted">Пробег</dt><dd className="mt-1 font-semibold">{item.mileage ? `${item.mileage.toLocaleString("ru-RU")} км` : "Не указан"}</dd></div><div className="col-span-2 min-w-0"><dt className="text-muted">VIN</dt><dd className="mt-1 break-all font-mono text-xs">{item.vin || "Не указан"}</dd></div></dl>
        ...
        <button className="mt-3 w-full text-center text-xs font-semibold text-apex" onClick={(event) => { event.stopPropagation(); void navigate(`/cars/${item.id}/history`); }}>Открыть историю автомобиля</button>
      </Card>;
