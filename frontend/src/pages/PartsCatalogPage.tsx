...
export function PartsCatalogPage() {
  ...
  if (status.isPending || crm.isPending || account.isPending) return <Spinner label="Открываю каталог…" />;
  if (!account.data?.platform_admin) return <EmptyState>Подбор и импорт из ROSSKO и Profit Liga доступны только владельцу платформы.</EmptyState>;
...
function RosskoImport({ order, crmOrderId, markup }: { order: RosskoOrder; crmOrderId: number; markup: number }) {
  ...
  return <div className="grid min-w-0 gap-3 rounded-xl border border-line bg-panel-soft p-3 sm:grid-cols-[1fr_auto] sm:items-center">
    <div className="min-w-0"><div className="flex min-w-0 flex-wrap items-center gap-2"><strong>ROSSKO №{order.id}</strong><span className="text-xs text-muted">{order.created_date}</span><span className="text-xs text-muted">{order.payment_status}</span></div><p className="mt-1 text-sm">{order.parts.length} поз. · закупка {money(order.total_price)}</p><div className="mt-2 grid gap-1.5">{order.parts.map((part) => { const disabled = !available.includes(part); const checked = selected.includes(part.article); return <label key={`${order.id}-${part.article}`} className={`grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-2 text-xs ${disabled ? "text-muted line-through" : "text-white"}`}><input className="mt-0.5" type="checkbox" checked={checked && !disabled} disabled={disabled || add.isPending || order.imported} onChange={() => setSelected((items) => checked ? items.filter((article) => article !== part.article) : [...items, part.article])} /><span className="min-w-0 break-words leading-snug">{part.brand} {part.name} × {part.quantity}</span><span className="shrink-0 whitespace-nowrap text-right text-muted">{money(part.purchase_price)}</span></label>; })}</div>{unavailable ? <p className="mt-1 text-xs text-muted">Недоступно: {unavailable}</p> : null}</div>
...
function ProfitImport({ order, crmOrderId, markup }: { order: ProfitLigaOrder; crmOrderId: number; markup: number }) {
  ...
  const unavailable = order.parts.length - available.length;
  return <div className="grid min-w-0 gap-3 rounded-xl border border-line bg-panel-soft p-3 sm:grid-cols-[1fr_auto] sm:items-center">
    <div className="min-w-0"><div className="flex min-w-0 flex-wrap items-center gap-2"><strong>Profit Liga №{order.id}</strong><span className="text-xs text-muted">{order.created_date}</span><span className="text-xs text-muted">{order.payment_status}</span></div><p className="mt-1 text-sm">{order.parts.length} поз. · закупка {money(order.total_price)}</p><div className="mt-2 grid gap-1.5">{order.parts.map((part) => { const disabled = !available.includes(part); const checked = selected.includes(part.article); return <label key={`${order.id}-${part.article}`} className={`grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-2 text-xs ${disabled ? "text-muted line-through" : "text-white"}`}><input className="mt-0.5" type="checkbox" checked={checked && !disabled} disabled={disabled || add.isPending || order.imported} onChange={() => setSelected((items) => checked ? items.filter((article) => article !== part.article) : [...items, part.article])} /><span className="min-w-0 break-words leading-snug">{part.brand} {part.name} × {part.quantity}</span><span className="shrink-0 whitespace-nowrap text-right text-muted">{money(part.purchase_price)}</span></label>; })}</div>{unavailable ? <p className="mt-1 text-xs text-muted">Недоступно: {unavailable}</p> : null}</div>
    <Button disabled={order.imported || add.isPending || add.isSuccess || !selected.length} onClick={() => add.mutate()}>{order.imported || add.isSuccess ? <><Check size={17} />Импортирован</> : <><Download size={17} />{selected.length === available.length ? "Добавить всё" : `Добавить выбранное (${selected.length})`}</>}</Button>
    {add.error && <p className="text-xs text-danger sm:col-span-2">Не удалось импортировать заказ. Возможно, он уже был добавлен.</p>}
  </div>;
}
